...
$mysqli = new mysqli("localhost","root","","vaccine");
...
if(count($errors)==0){
	...
	$sql = "INSERT INTO  bookapp (AppoID, Date, Time, parentID,docID) VALUES ('$AppoID','$Date','$Time','$userprofile','$docID') ";
...
if (isset($_POST['cancel'])) {
 ...
 if (count($errors)==0) {
	...
	$query5="DELETE FROM bookapp WHERE AppoID='$AppoID2' AND parentID=('$userprofile') ";
...
if(count($errors)==0){
		...
		$addlocation 	= $_REQUEST['addlocation'];


	$sqladd = "INSERT INTO  doctor (DoctorID, Doctorname, email, Address, ContactNumber, password,location) VALUES ('$addID','$addname','$addEmail','$addAddress','$addContactNumber','$addPassword','$addlocation') ";
...
<!-- $query1="SELECT location FROM doctor";
   $result1= mysqli_query($mysqli, $query1);
   $opt="<select name='Location'>";
   while ($row= mysqli_fetch_assoc($result1)) {
   	$opt.="<option value ='{$row['location']}'>{$row['location']}</option>";
   }
   	$opt.="</select>";
